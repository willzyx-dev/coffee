...
Drupal.behaviors.coffee = function () {
      var body = $('body');
...
Drupal.coffee.field = $('<input id="coffee-q" type="text" autocomplete="off" />').keyup(function () {
  ...
  // @todo Find a solution for xhr results coming in later. 
  if (Drupal.coffee.field.val().length > 0) {
    req = $.getJSON(Drupal.settings.basePath + 'admin/coffee/result/' + Drupal.coffee.field.val(), function (data) {
      if (data) {
